...
function multiplyBy(arr, multiplier) {
  ...
  arr.forEach((num) => multipliedArr.push(num * multiplier));
...
function filterOut(original, toRemove) {
  ...
  const cleanArr = [];
  bullets: for (let firstWord of original) {
    for (let secondWord of toRemove) {
      if (firstWord === secondWord) {
        continue bullets;
      }
    }
    cleanArr.push(firstWord);
  }
  return cleanArr;
}
...
function uniquifyArray(originalArr) {
  if (originalArr.length === 0) return null;

  const cleanArr = [];

  for (word of originalArr) {
